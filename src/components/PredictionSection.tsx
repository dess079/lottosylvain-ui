...
const PredictionSection: React.FC = () => {
            ...
            <div key={number} className="relative">
              <button
                className={`w-14 h-14 rounded-full flex items-center justify-center font-medium transition-all duration-200 text-lg shadow-lg ${
                  isSelected ? 'bg-primary-200 text-primary-800 border-3 border-primary-500 shadow-md scale-110' :
                  isExcluded ? 'bg-gray-200 text-gray-500 border-3 border-gray-500 line-through opacity-70' :
    ...
    return (
      <div className="flex flex-wrap gap-3 justify-center my-6">
        {prediction.numbers.map((number, index) => (
          ...
          >
            <LottoBall number={number} animate={true} />
          </motion.div>
        ))}
      </div>
    );
  ...
  const renderConfidenceBar = (score: number) => {
    ...
    // Nouvelle échelle de couleurs avec dégradé amélioré
    const getColor = () => {
      ...
      <div className="w-full mb-5 mt-3">
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex items-center">
            <span className={`px-3 py-1 rounded-full text-xs font-bold text-white bg-gradient-to-r ${getColor()} shadow-sm`}>
      ...
      <Card className="max-w-4xl mx-auto p-8 card-shimmer shadow-lg">
        <div className="flex flex-col items-center">
          ...
          <Tabs defaultValue="standard" className="w-full mb-8">
            <TabsList className="grid w-full grid-cols-2 p-1 rounded-xl bg-gray-100 dark:bg-gray-800 mb-6">
              <TabsTrigger value="standard" className="rounded-lg py-3 font-bold text-base">Prédictions standard</TabsTrigger>
              <TabsTrigger value="custom" className="rounded-lg py-3 font-bold text-base">Prédictions personnalisées</TabsTrigger>
            </TabsList>
              ...
              <div className="w-full">
                <div className="flex justify-center mb-8">
                  <Button 
                    ...
                    disabled={isLoading}
                    size="lg"                      className="relative px-10 py-6 text-base font-bold shadow-xl hover:shadow-2xl transition-shadow"
                  >
                    {isLoading ? (
                      <>
                    ...
                    {predictions.map((prediction, index) => (
                      <motion.div
                        ...
                        className="p-8 border border-gray-200 rounded-xl 
                          ...
                          before:absolute before:inset-0 before:rounded-xl before:bg-gradient-to-br before:from-white/10 before:to-transparent before:content-['']
                          dark:before:from-gray-800/10 dark:before:to-transparent"
                      >
                        <h3 className="text-2xl font-bold mb-4 text-primary-600 dark:text-primary-400">Combinaison #{index + 1}</h3>
                ...
                <div className="mb-10 p-8 bg-gray-50/90 dark:bg-gray-800/60 rounded-xl shadow-inner border border-gray-200 dark:border-gray-700 analysis-box">
                  ...
                  <div className="mb-8">
                    <label className="block text-base font-medium text-gray-700 dark:text-gray-300 mb-3">
                      Importance des données historiques: <span className="font-bold text-primary-600 dark:text-primary-400">{historicalWeight}%</span>
                    </label>
                    <input
                      type="range"
                      min="0"
                    ...
                    {customPredictions.map((prediction, index) => (
                      <motion.div
                        ...
                        className="p-8 border border-gray-200 rounded-xl 
                          ...
                          dark:before:from-gray-800/10 dark:before:to-transparent"
                      >
                        <h3 className="text-2xl font-bold mb-4 text-primary-600 dark:text-primary-400">Combinaison personnalisée #{index + 1}</h3>
                        {renderPredictionBalls(prediction)}
                        {renderConfidenceBar(prediction.confidenceScore)}
